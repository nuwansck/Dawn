...
def validate_settings(settings: dict) -> dict:
    ...
    settings.setdefault("max_trades_london",          1)
...
def compute_sl_usd(levels: dict, settings: dict) -> float:
    ...
    Modes:
      range_based : Dawn — SL = signal levels.sl_usd_rec (range × sl_range_pct)
    ...
    if sl_mode == "range_based":
        # Dawn: signal engine pre-computes SL from session range.
        sl_rec = levels.get("sl_usd_rec")
        if sl_rec is not None:
            try:
                sl_val = float(sl_rec)
                if sl_val > 0:
                    return round(sl_val, 2)
            except (TypeError, ValueError):
                pass
        log.warning("range_based SL: sl_usd_rec missing — falling back to fixed_usd")
...
def compute_tp_usd(levels: dict, sl_usd: float, settings: dict) -> float:
    ...
    # Dawn: range-based TP comes directly from signal engine (range × tp_pct).
    # Cap at max_rr_ratio × sl_usd so clamped SLs don't produce silly RR.
    if tp_mode == "range_based":
        tp_rec = levels.get("tp_usd_rec")
        if tp_rec is not None:
            try:
                tp_val = float(tp_rec)
                if tp_val > 0:
                    return round(min(tp_val, tp_ceil), 2)
            except (TypeError, ValueError):
                pass
        # Missing tp_usd_rec — fall back to rr_multiple
    ...
    if result.get("success"):
        ...
        if ctx.get("is_pyramid"):
            ...
            alert.send(msg_trade_opened(
                ...
                range_size=levels.get("range_size"),
                sl_raw_usd=levels.get("sl_raw_usd"),
            ))
